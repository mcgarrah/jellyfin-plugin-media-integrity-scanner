...
# Environment bring-up (Docker readiness, startup wizard, auth, test-media
# library) lives in setup-jellyfin.sh, shared with the Playwright suite's
# global setup -- source it here to inherit $TOKEN/$JELLYFIN_URL/$PLUGIN_GUID
# and the pass/fail/info helpers.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/setup-jellyfin.sh"
...
# even after setup-jellyfin.sh's settle-check) instead of failing outright.
HTTP_CODE=""
